...
class RolloutBuffer:
    ...
    def finish_episode(self, final_value: float, gamma: float = 0.99, gae_lambda: float = 0.95):
        """Compute returns and advantages using GAE for current episode and accumulate."""
        ...
        # ACCUMULATE returns and advantages instead of replacing
        self.returns.extend(returns.tolist())
        self.advantages.extend(advantages.tolist())

        # Clear episode-specific data (but keep accumulated returns/advantages)
        self.rewards = []
        self.values = []
...
class SyntheticTradingEnv:
    ...
    def reset(self) -> Tuple[np.ndarray, str]:
        ...
        # Get regime type (use 'type' field which has scenario type)
        regime_type = self.current_scenario.get("type", self.current_scenario.get("regime", "mixed"))
    ...
    def step(self) -> Tuple[np.ndarray, float, float, bool, str]:
        ...
        if self.step_idx >= len(self.current_scenario["returns"]) - 1:
            ...
            # Use 'vols' field from synthetic generator, fallback to 'volatility' array or default
            if "vols" in self.current_scenario and self.step_idx < len(self.current_scenario["vols"]):
                volatility = self.current_scenario["vols"][self.step_idx]
            elif "volatility" in self.current_scenario:
                vol_data = self.current_scenario["volatility"]
                if isinstance(vol_data, np.ndarray) and self.step_idx < len(vol_data):
                    volatility = vol_data[self.step_idx]
        ...
        # Use 'type' field for regime (scenario type), fallback to 'regime' field
        regime = self.current_scenario.get("type", self.current_scenario.get("regime", "mixed"))
...
def main():
    ...
    while total_steps < args.steps:
        ...
        # End episode - compute returns and advantages
        episode_stats = trainer.end_episode()
        episode_sharpe = episode_stats["episode_sharpe"]

        # PPO update after episode ends (when returns/advantages are computed)
