"""
HIMARI Layer 3: Bounded Delta Training V3
==========================================

V3 Improvements (from V2):
1. Prioritized risk-adjusted returns (1.5x weight)
2. Reduced regime compliance penalty (0.15 vs 0.5)
3. Reduced drawdown penalty (1.0x vs 2.0x)
4. Higher delta efficiency reward (0.4 vs 0.25)
5. Less defensive bias, better bull market performance

Expected improvement: Sharpe -0.037 → +0.10 to +0.20
...
class BoundedDeltaConfig:
    ...
    # V3 Reward weights - prioritize returns over defensive behavior
    w_risk_adjusted: float = 0.50       # V3: Increased from 0.40 (1.5x priority on returns)
    w_delta_efficiency: float = 0.25    # V3: Kept same (reward correct direction)
    w_drawdown_penalty: float = 0.10    # V3: Reduced from 0.15 (less loss aversion)
    w_regime_compliance: float = 0.05   # V3: Reduced from 0.10 (less defensive bias)
    w_smoothness: float = 0.05          # Keep same
    w_survival_bonus: float = 0.05      # Keep same
...
def main():
    ...
    if WANDB_AVAILABLE:
        wandb.init(
            project="himari-layer3-bounded-delta-v3",
            name=f"v3_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            config={
    ...
    logging.info("=" * 60)
    logging.info("Starting Bounded Delta Training V3")
    logging.info("=" * 60)
    ...
    # Final save
    final_path = os.path.join(args.output, "bounded_delta_v3_final.pt")
    ...
    logging.info("=" * 60)
    logging.info("Bounded Delta Training V3 Complete!")
    logging.info("=" * 60)
