#!/usr/bin/env python3
"""
HIMARI Layer 3 - LSTM Vertex AI Training Script
Trains LSTM-based PPO agent with temporal memory
"""
...
class LSTMVertexAITrainer:
    """Vertex AI training wrapper for LSTM-PPO Agent"""
    ...
    def __init__(
        ...
        logger.info(f"Initialized LSTM VertexAITrainer")
        logger.info(f"  Bucket: {bucket_name}")
        logger.info(f"  Model dir: {model_dir}")
        logger.info(f"  Episodes: {num_episodes}")

    def create_lstm_ppo_config(self) -> LSTMPPOConfig:
        """Create LSTM-PPO configuration for training"""
        return LSTMPPOConfig(
            state_dim=16,
            action_dim=1,
            hidden_dim=128,
            lstm_hidden_dim=64,
    ...
    def train(self):
        ...
        lstm_ppo_config = self.create_lstm_ppo_config()
        ...
        for episode in range(self.num_episodes):
            ...
            if (episode + 1) % self.save_interval == 0:
                checkpoint_path = self.local_model_dir / f"lstm_ppo_ep{episode+1}.pt"
                agent.save(str(checkpoint_path))

                gcs_checkpoint_path = f"{self.model_dir}/lstm_ppo_ep{episode+1}.pt"
                self.upload_to_gcs(checkpoint_path, gcs_checkpoint_path)

        logger.info("Training completed!")

        # Save final model
        final_model_path = self.local_model_dir / "lstm_ppo_final.pt"
        agent.save(str(final_model_path))
        logger.info(f"Final model saved: {final_model_path}")

        # Upload to GCS
        gcs_model_path = f"{self.model_dir}/lstm_ppo_final_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pt"
        self.upload_to_gcs(final_model_path, gcs_model_path)

        # Upload as latest
        gcs_latest_path = f"{self.model_dir}/lstm_ppo_latest.pt"
...
def main():
    """Main entry point for LSTM Vertex AI training"""
    parser = argparse.ArgumentParser(description='HIMARI Layer 3 LSTM Vertex AI Training')
    ...
    logger.info("="*60)
    logger.info("HIMARI Layer 3 - LSTM Vertex AI Training")
    logger.info("="*60)
