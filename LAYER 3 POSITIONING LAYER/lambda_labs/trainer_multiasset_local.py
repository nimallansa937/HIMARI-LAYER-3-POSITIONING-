#!/usr/bin/env python3
"""
HIMARI Layer 3 - Lambda Labs Multi-Asset Local Training Script
Trains Multi-Asset PPO agent locally without GCS authentication
"""
...
class MultiAssetLocalTrainer:
    """Local Multi-Asset training wrapper for Lambda Labs"""
    ...
    def __init__(
        ...
        logger.info(f"Initialized Multi-Asset LocalTrainer")
    ...
    def train(self):
        ...
        env_config = MultiAssetEnvConfig(
        ...
        ppo_config = PPOConfig(
            ...
            hidden_dim=256,  # Larger for multi-asset
            learning_rate=3e-4,
            gamma=0.99,
            lambda_gae=0.95,
            clip_epsilon=0.2,
        ...
        agent = PPOAgent(ppo_config, device=self.device)
        ...
        for episode in range(self.num_episodes):
            ...
            while not done:
                ...
                for symbol in self.symbols:
                    if info['positions'].get(symbol, 0) != 0:
            ...
            if (episode + 1) % self.save_interval == 0:
                checkpoint_path = self.model_dir / f"multiasset_ppo_ep{episode+1}.pt"
        ...
        # Save final model
        final_model_path = self.model_dir / "multiasset_ppo_final.pt"
        agent.save(str(final_model_path))
        logger.info(f"Final model saved: {final_model_path}")

        # Save timestamped version
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        timestamped_path = self.model_dir / f"multiasset_ppo_final_{timestamp}.pt"
        ...
        for symbol, performance in asset_performances.items():
...
def main():
    parser = argparse.ArgumentParser(description='HIMARI Layer 3 Multi-Asset Local Training')

    parser.add_argument('--symbols', type=str, nargs='+', default=['BTC-USD', 'ETH-USD', 'SOL-USD'])
    ...
    logger.info("=" * 60)
    logger.info("HIMARI Layer 3 - Multi-Asset Local Training")
    logger.info("=" * 60)
    ...
    trainer = MultiAssetLocalTrainer(
